...
### Consider the following pseudocode implementation of the spinlock-free semaphore wait operation taken from the lecture slides. An error has been introduced in one of the lines, which one?
...
- A) A deadlock detection algorithm is called for each resource request
- B) Deadlocks are only affecting threads
- C) A resource-allocation graph can be used to detect deadlocks ✅
- D) No deadlock is possible if any of the Coffman conditions are false ✅

### Which of the following operations require a system call to be performed when called from a user program?

- A) Reading from memory allocated on the heap
- B) Allocating new memory on the heap ✅
- C) Reading from a file on disk ✅
- D) Making a regular function call
...
- A) A deadlock may occur if another process invokes wait on S1 and S2 in the wrong order. ✅
- B) A deadlock may occur if another process invokes signal on S1 and S2 in the wrong order.
- C) A race condition may occur if another process omits to call wait on one of the semaphores. ✅
- D) A race condition may occur if another process spuriously calls signal on one of the semaphores. ✅
